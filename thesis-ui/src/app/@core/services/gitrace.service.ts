...
export class GitraceService {
  ...
  findAll(): Observable<Array<Gitrace>> {
    return this.httpClient.get<Array<Gitrace>>(`${this.endpoint}/gitrace/`, {
      headers: {
  ...
  create(gitrace: Gitrace): Observable<Gitrace> {
    return this.httpClient.post<Gitrace>(`${this.endpoint}/gitrace/`, gitrace, {
      headers: {
        'Authorization': localStorage.getItem(LOCALSTORAGE_KEY_TOKEN)
      }
    });
